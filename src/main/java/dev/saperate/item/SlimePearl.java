...
public class SlimePearl extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!user.isSneaking()) { //Launching pearl
            ...
            if (!world.isClient) {
                ...
                int numBounces = SapsUtils.getCustomInt(handStack, "numBounces");
            ...
            int diff = maxBounce - SapsUtils.getCustomInt(handStack, "numBounces");
            int count = Math.min(offHandStack.getCount(), diff);

            if (!user.getAbilities().creativeMode) {
                offHandStack.decrement(count);
            }
            SapsUtils.setCustomInt(handStack, SapsUtils.getCustomInt(handStack, "numBounces") + count, "numBounces");
        }
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        addToTooltip(tooltip, "item.sapswackystuff.slime_pearl.tooltip", SapsUtils.getCustomInt(stack, "numBounces"));
    }
